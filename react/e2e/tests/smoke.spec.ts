...
test.describe('App shell (unseeded)', () => {
  test('boots into the dashboard in local-only mode', async ({ page, dashboard }) => {
    await page.goto('/');
  ...
  test('does not render a cloud auth screen', async ({ page }) => {
    await page.goto('/');
...
test.describe('Seeded household', () => {
  ...
  test('seeded transactions are visible on the Transactions page', async ({ transactions }) => {
  ...
  test('seeded data survives a full page reload (persistence guard)', async ({ page, transactions }) => {
    await transactions.goto();
    await expect(transactions.row('E2E Salary')).toBeVisible();
    await page.reload();
