...
| TD-04 | ✅ Admin privilege schema not version-controlled *(resolved · PR #7 + #13 ext + #14 read RPCs)* | Security | High | S |
...
| 2026-05-24 | #14 | **TD-04 final — closes the family** (Database `admin-read-rpcs-and-ai-usage`). Ships the three originally-requested admin READ RPCs (`admin_list_users`, `admin_weekly_trend(weeks)`, `admin_ai_usage_summary`) that the dev's PR #13 batch skipped in favour of an unrelated set of mutation RPCs. The Intelligence-page-required `ai_usage` table (privacy-safe: `user_id`, `intent`, `sentiment_score`, `message_length`; never content) is included with RLS (a user writes/reads their own rows; admins read all). All three RPCs are SECURITY DEFINER with `search_path = public, pg_temp` lockdown and `is_admin()` guards that raise `42501` on non-admin callers. `admin_weekly_trend` bounds the lookback at 1..104 weeks. `admin_ai_usage_summary` derives the bySentiment label from `sentiment_score` using the same `< -0.15` / `> 0.15` thresholds the admin Intelligence page uses. | Closes **TD-04** completely (marked ✅ Resolved in the Summary table). Note: nothing else in the codebase was modified — the consumer app and admin app already had the fetchers wired; this migration just delivers what they were waiting for. The `ai_usage` Edge Function that writes rows (consumer-side) is the natural follow-up but lives in TD-07 (AI backend), not TD-04. |
